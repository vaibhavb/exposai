...
import { confirm, openEditor, askQuestion, substituteVariables, logger } from './utils.js';

export class ArticleGenerator {
    constructor(exposaiState, model='ollama', modelOpts={}) {
    ...
    async getResponse(prompt) {
        ...
        else if (this.modelName === 'anthropic') {
        ...
        logger.prompt("fn", prompt, output);
        return output;
    }
    ...
    async restartPreviousRun() {
        logger.info('Restarting previous run with existing state...');
    ...
    async generateArticle() {
        ...
        for (const step in steps) {
            ...
            if (typeof this[methodName] === 'function') { // Check if the function exists globally
                ...
                logger.info(methodName, this.exposaiState[step]); // Log the function call and output
    ...
    async genTitle() {
        ...
        if (res === 'yes') {
            ...
            titlePrompt = await openEditor('Enter new prompt:\n');
            return titlePrompt;
        }
    }
    ...
    async genDataSchema(){
        // Open your database
         return `
                CREATE TABLE "candidates" (
                  ...
                  "Constituency" TEXT,
                  "Party" TEXT,
                  "Criminal Cases" INTEGER,
                  "Education" TEXT,
                  "Total Assets" INTEGER,
                  "Total Liabilities" INTEGER
                )
                CREATE TABLE loksabhaconstituencies (
                    Constituency TEXT,
        ...
        await db.all("PRAGMA table_info('candidates')", [], async (err, rows) => {
            ...
            schema = `Schema: ${columnNames.join(',')}\n`;
        })
        logger.info(schema);
        // Return schema
        return schema;
